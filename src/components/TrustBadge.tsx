import { Shield, Lock, Download } from "lucide-react";

interface TrustBadgeProps {
  variant?: "tdog" | "secure" | "download";
  className?: string;
}

export function TrustBadge({ variant = "tdog", className = "" }: TrustBadgeProps) {
  if (variant === "tdog") {
    return (
      <div className={`flex items-start gap-4 ${className}`}>
        <div className="shrink-0">
          <img
            src="/brand/t-dog-certified.png"
            alt="T-DOG Certified"
            className="h-12 w-12 object-contain"
            onError={(e) => {
              const target = e.target as HTMLImageElement;
              target.style.display = 'none';
              const parent = target.parentElement!;
              parent.innerHTML = '<div class="h-12 w-12 rounded-full bg-gold/20 flex items-center justify-center"><svg class="h-6 w-6 text-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg></div>';
            }}
          />
        </div>
        <div className="text-left">
          <p className="text-sm font-semibold text-foreground mb-1">T-DOG Certified</p>
          <p className="text-xs text-muted-foreground leading-relaxed">
            QC-checked photoreal results. If it doesn't look like you, we regenerate or refund.
          </p>
        </div>
      </div>
    );
  }

  if (variant === "secure") {
    return (
      <div className={`flex items-start gap-4 ${className}`}>
        <div className="h-12 w-12 shrink-0 rounded-full bg-gold/10 flex items-center justify-center">
          <Lock className="h-5 w-5 text-gold" />
        </div>
        <div className="text-left">
          <p className="text-sm font-semibold text-foreground mb-1">Secure Payments</p>
          <p className="text-xs text-muted-foreground">256-bit SSL encryption</p>
        </div>
      </div>
    );
  }

  return (
    <div className={`flex items-start gap-4 ${className}`}>
      <div className="h-12 w-12 shrink-0 rounded-full bg-gold/10 flex items-center justify-center">
        <Download className="h-5 w-5 text-gold" />
      </div>
      <div className="text-left">
        <p className="text-sm font-semibold text-foreground mb-1">30-Day Access</p>
        <p className="text-xs text-muted-foreground">Download anytime</p>
      </div>
    </div>
  );
}

export function TrustStrip() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 md:p-8 bg-secondary/50 rounded-2xl border border-border/50">
      <TrustBadge variant="tdog" />
      <TrustBadge variant="secure" />
      <TrustBadge variant="download" />
    </div>
  );
}

export default TrustBadge;
